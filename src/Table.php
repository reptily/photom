        ...
        public function find(int $id){
            ...
            return new class($this->model,$this->table,$key,$this->DB,$id){
                ...
                public function __call($name,$arg){
                        ...
                        $method = mb_strtolower($a[1]);
                        switch($a[0]){
                                case "get":
                                        return $this->model[$method];
                                        break;
                                case "set":
                                        $this->model[$method]=mysqli_real_escape_string($this->DB->connectId,$arg[0]);
                                        $this->DB->{$this->table}->set([$method=>$arg[0]])->where([$this->key=>$this->id])->Update();
                                        break;
                                default:
                                        exit("Can not method");
                        }
